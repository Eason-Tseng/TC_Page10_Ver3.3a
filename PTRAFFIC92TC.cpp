  ...
  if (bMollacOK) {
    ...
    for(int j = 0; j < iSubPhaseCount; j++) {
      ...
      for(int qq=0; qq<iSignalCount; qq++) {
        ucStepSignal[qq] = DataMessageIn.packet[iSignalCount*j+13+qq];   //����P�@�ɬۤ��O�����array
      }
...
try{
  ...
  data[iNumWeekDayPrt] = iNumWeekDay;                                           //�̫�ɤWNumWeekDay����
...
*/
    ...
    for(int j = 0; j < iSubPhaseCount; j++) {
      ...
      for( int k=0; k< iSubPhaseStepCount; k++ ) {
        ...
        for(int qq=0; qq<iSignalCount; qq++) {
          ucStepSignal[qq] = DataMessageIn.packet[iDataPtr];   //����P�@�ɬۤ��O�����array
          iDataPtr++;
        }
...
bool PTRAFFIC92TC::vSetActuatedType_5F19(MESSAGEOK DataMessageIn) //202009針對竹縣盲人號誌開發,只實作行人觸動部份 by Eason
{
  try
  {
	  ...
	  DATA_Bit ActuateType;//觸動類型
    ActuateType.DBit = DataMessageIn.packet[10];
    unsigned short int TimeExtend = DataMessageIn.packet[11];
	  unsigned short int tmpActPlan = DataMessageIn.packet[12];// ActuateData1 觸動發生所要執行之計畫編號
...
bool PTRAFFIC92TC::vQueryActuatedType_5F49(MESSAGEOK DataMessageIn) //202009針對竹縣盲人號誌開發,只實作行人觸動部份 by Eason
{
  try
  {
    ...
    data[5] = smem.vGetUSIData(TC_CCT_In_LongTanu_ActuateType_Switch); // ActuateData1 觸動發生所要執行之計畫編號
    data[6] = 0x00; //ActuateData2 未定義
